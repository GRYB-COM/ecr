 ...
 try{
  try{
   ...
   try{
		...
		if(m_Client->Socket) m_Client->Socket->Close();
		if(m_Client->IOHandler) m_Client->IOHandler->Close();
		if(m_Client->Socket && !m_Client->Socket->Opened) m_Client->Socket->Open();
		if(m_Client->IOHandler && !m_Client->IOHandler->Opened) m_Client->IOHandler->Open();

	  }
   m_Client->IOHandler->Write(_Buff,_Buff.Length);
   if(m_Obs) m_Obs->runOnECR(_Mess);
   //m_Timer->Enabled = true;
   m_Size    =0;
   m_MessStr="";
   bool _Err(true);
   int _TimeOut(m_CloseMess.getTimeOut());
   if(_TimeOut <1) _TimeOut = Globals::TIME_B;
   ...
   if(_Err) throw ECRTimeOutException();
  }
...
void TerminalIndy::resp(const Message& __Mess)
{
 try{
  ...
  Message _Mess(__Mess);
   //m_Timer->Enabled = false;
   TIdBytes _Buff(getMessAsBytes(_Mess));
   if(!m_Client->Connected() ) m_Client->Connect();
   m_Client->IOHandler->Write(_Buff,_Buff.Length);
   if(m_Obs) m_Obs->runOnECR(_Mess);
   //m_Timer->Enabled = true;
  }
...
void  TerminalIndy::listen(int& _Counter)
{
  ...
  if(!m_Client->IOHandler->InputBufferIsEmpty() )
  {
   ...
   while(!m_Client->IOHandler->InputBufferIsEmpty())
   {
	...
	if( static_cast<unsigned short>(m_MessStr.Length()) >= m_Size+2)
	{
	 ...
	 if(m_Obs) m_Obs->runOnMsg(m_Mess);
	 m_Size=0;
	 m_MessStr="";
 ...
 if(m_Obs) m_Obs->runOnListen(_Counter);
  ////m_Timer->Enabled = false;
 Application->ProcessMessages();
 }
 catch(...){
